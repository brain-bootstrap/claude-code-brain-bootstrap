...
> 💡 **Full Copilot parity:** If you installed with `--copilot`, you also get 5 custom agents (`@reviewer`, `@researcher`, etc.), 37 slash commands, and 4 lifecycle hooks — all auto-generated from the Claude equivalents.

For **full setup** (filling in the knowledge docs — architecture, build commands, project name, etc.), use the `#bootstrap` prompt generated by `--copilot`:

**Option A — with `--copilot` install** (recommended):

`#bootstrap` is already in `.github/prompts/bootstrap.prompt.md`. Open Copilot Chat and type:

```
/bootstrap
```

Copilot will run the discovery engine, detect your entire stack, fill all `{{PLACEHOLDERS}}` in `claude/architecture.md`, `claude/build.md`, `CLAUDE.md`, and list anything it couldn't resolve. Same flow as `/bootstrap` in Claude Code.

**Option B — without `--copilot`** (manual paste):

```bash
# Run discovery first (pure bash, 2 seconds, zero tokens)
