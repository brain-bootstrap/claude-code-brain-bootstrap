...
- `plugins` — Re-run Phase 4 only: **ask the user** which strategy they want (none/full/recommended/personalize), then run `bash claude/scripts/setup-plugins.sh --strategy=<choice> .`. **⚠️ NEVER run without `--strategy=` flag** — it hangs on interactive prompts inside AI agents. Safe to re-run (idempotent).
